...
function getStoreConfig_() {
  ...
  const raw =
    String(
      props.getProperty('STORE_CONFIG_JSON') || ''
    ).trim();
...
const GITHUB_PROD_OWNER_ =
  String(
    getStoreConfig_().GITHUB_OWNER || ''
  ).trim();

const GITHUB_PROD_REPO_ =
  String(
    getStoreConfig_().GITHUB_REPO || ''
  ).trim();

const GITHUB_PROD_BRANCH_ =
  String(
    getStoreConfig_().GITHUB_BRANCH || 'main'
  ).trim();

const GITHUB_PROD_IMAGE_DIR_ =
  String(
    getStoreConfig_().GITHUB_IMAGE_DIR ||
    'images/menu'
  ).trim();
